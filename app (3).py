...
PCFG = dict(
    ...
    title_font_color="#111827", title_font_weight="bold",
    ...
    try:
        ...
        if "dir_1" in df.columns and df["dir_1"].notna().sum()>0:
            ...
            if "dir_2" in df.columns:
                ss["Top Sub-sections"] = ss["Section"].apply(lambda d1: ", ".join([f"{k}({v})" for k,v in df[df["dir_1"]==d1]["dir_2"].dropna().value_counts().head(3).items()]) or "leaf pages")
            st.dataframe(ss, use_container_width=True, hide_index=True)
    except Exception as e:
        st.error(f"URL Structure error: {e}")

# ═══════════════════════════════════════
# TAB 2 — SITE HIERARCHY (expander tree)
# ═══════════════════════════════════════
with tabs[1]:
    try:
        sh("Site Hierarchy",
           insight="Wide shallow trees = topic authority play. Deep narrow trees = long-tail strategy.")
        ...
        tree = build_tree(tuple(map(tuple, df["url_parts"].tolist())))

        def get_children(node, min_c=1):
            ch = [(k,v) for k,v in node.items() if k!="__count__" and v.get("__count__",0)>=min_c]
            return sorted(ch, key=lambda x: x[1].get("__count__",0), reverse=True)

        min_c = st.number_input("Min URLs per node", min_value=1, value=2, step=1, key="hier_min")

        root_count = tree.get("__count__", n_total)
        st.markdown(f"**🌐 {domain_name}** — {root_count:,} URLs")
        st.markdown("---")

        level1 = get_children(tree, int(min_c))
        for sec, sec_node in level1:
            sec_count = sec_node.get("__count__", 0)
            sec_pct = round(sec_count/n_total*100, 2)
            level2 = get_children(sec_node, int(min_c))
            label = f"📁 {sec}   —   {sec_count:,} URLs   ({sec_pct}%)"
            if level2:
                with st.expander(label):
                    for sub, sub_node in level2:
                        sub_count = sub_node.get("__count__", 0)
                        sub_pct = round(sub_count/n_total*100, 2)
                        level3 = get_children(sub_node, int(min_c))
                        sub_label = f"📂 {sub}   —   {sub_count:,} URLs   ({sub_pct}%)"
                        if level3:
                            with st.expander(sub_label):
                                for sub2, sub2_node in level3:
                                    sub2_count = sub2_node.get("__count__", 0)
                                    sub2_pct = round(sub2_count/n_total*100, 2)
                                    level4 = get_children(sub2_node, int(min_c))
                                    sub2_label = f"📄 {sub2}   —   {sub2_count:,} URLs   ({sub2_pct}%)"
                                    if level4:
                                        with st.expander(sub2_label):
                                            rows = [{"Path": k, "URLs": v.get("__count__",0),
                                                     "%": f"{round(v.get('__count__',0)/n_total*100,3)}%"}
                                                    for k,v in level4]
                                            st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
                                    else:
                                        st.markdown(f"&nbsp;&nbsp;&nbsp;&nbsp;{sub2_label}", unsafe_allow_html=True)
                        else:
...
with tabs[4]:
    try:
        ...
        if n_dates > 0:
            ...
            rec.columns = ["Depth","Avg Days Since Update"]
            tot = df.groupby("url_depth").size().reset_index(name="Total URLs")
            rec = rec.merge(tot, on="Depth", how="outer").fillna(0)
            rec["Avg Days Since Update"] = rec["Avg Days Since Update"].astype(int)
            rec = rec.sort_values("Depth")
            rec["SEO Risk"] = rec.apply(
                lambda r: "🔴 Stale+Deep" if r["Depth"]>=3 and r["Avg Days Since Update"]>365
...
<div class="footer">Built by <a href="https://www.linkedin.com/in/sankar-gurumurthy-a1044a136/">Sankar Gurumurthy</a> · <a href="https://github.com/sg-sankar">github.com/sg-sankar</a> · Sitemap Copilot · Open source &amp; free forever</div>
        ...
        with c2:
            dcols2 = display_cols(df)
            st.download_button("⬇ Download Full CSV", clean_df(df)[dcols2].to_csv(index=False).encode("utf-8"),
                               f"sitemap_{domain_name.replace('.','_')}.csv", "text/csv")

        st.markdown("---")
        st.markdown("**Report includes:** Overview metrics · URL depth distribution · Site structure with sub-sections · N-gram analysis · Freshness breakdown · Content gap opportunities")
        st.markdown("**Does not include:** Individual URL slugs (noise) — use CSV for full URL list")
