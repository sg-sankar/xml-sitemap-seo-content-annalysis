...
with tabs[1]:
    try:
        sh("Site Hierarchy",
           insight="Wide shallow trees = topic authority play. Deep narrow trees = long-tail strategy. Click any row to expand.")
        ...
        def get_children(node, min_c=1):
            ch = [(k,v) for k,v in node.items() if k!="__count__" and isinstance(v,dict) and v.get("__count__",0)>=min_c]
        ...
        st.markdown(f"""
        <div style="font-size:0.95rem;font-weight:700;color:#111827;padding:0.75rem 1rem;
        background:#f0f7ff;border:1px solid #bfdbfe;border-radius:8px;margin-bottom:0.5rem">
            ...
            with st.expander(f"📁  {sec}     {sec_count:,} URLs  ({sec_pct}%)"):
                if level2:
                    for sub, sub_node in level2:
                        ...
                        # Level 2 expander (inside level 1)
                        with st.expander(f"📂  {sub}     {sub_count:,} URLs  ({sub_pct}%)"):
                            if level3:
                                for sub2, sub2_node in level3:
                                    ...
                                    # Level 3 expander (inside level 2)
                                    with st.expander(f"📄  {sub2}     {sub2_count:,} URLs  ({sub2_pct}%)"):
                                        if level4:
                                            rows = []
                                            for k, v in level4:
                                                cnt = v.get("__count__", 0)
                                                rows.append({"Name": k, "URLs": cnt, "%": f"{round(cnt/n_total*100,3)}%"})
                                            st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
                                        else:
                                            st.caption("Leaf node — no further sub-directories")
                            else:
                                st.caption("Leaf node — no further sub-directories")
                else:
                    st.caption("No sub-directories found under this section")
